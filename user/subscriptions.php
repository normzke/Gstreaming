...
<?php if (!$currentSubscription || $currentSubscription['current_status'] !== 'active'): ?>
    <div
        style="background: linear-gradient(135deg, #8B0000, #660000); color: white; padding: 2rem; border-radius: 12px; margin-bottom: 2rem; text-align: center;">
...
<div class="user-card">
    ...
    <div class="card-body">
        <?php if ($currentSubscription && $currentSubscription['current_status'] === 'active'): ?>
            <div
                style="background: linear-gradient(135deg, #10B981, #059669); color: white; padding: 2rem; border-radius: 12px; margin-bottom: 2rem;">
                <div
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem;">
                        <div>
                            ...
                            <p style="margin: 0; font-weight: 600;">
                                <?php echo date('M d, Y', strtotime($currentSubscription['start_date'])); ?>
                            </p>
                        </div>
                        <div>
                            <p style="margin: 0; opacity: 0.9; font-size: 0.85rem;">End Date</p>
                            <p style="margin: 0; font-weight: 600;">
                                <?php echo date('M d, Y', strtotime($currentSubscription['end_date'])); ?>
                            </p>
                        </div>
                        <div>
                            <p style="margin: 0; opacity: 0.9; font-size: 0.85rem;">Duration</p>
                            <p style="margin: 0; font-weight: 600;"><?php echo $currentSubscription['duration_days']; ?>
                                Days</p>
                        </div>
                    </div>
                </div>
            ...
            <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                ...
                <a href="/user/subscriptions/subscribe?package=<?php echo $currentSubscription['package_id']; ?>"
    ...
    <div class="card-body">
        ...
        <div style="margin-bottom: 2rem; text-align: center;">
            ...
            <div style="display: flex; gap: 0.5rem; justify-content: center; flex-wrap: wrap;">
                <button class="device-tab-btn active" data-devices="1"
                    style="padding: 0.75rem 1.5rem; border: 2px solid var(--user-primary); background: var(--user-primary); color: white; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s;">
                    <i class="fas fa-mobile-alt"></i> 1 Device
                </button>
                <button class="device-tab-btn" data-devices="2"
                    style="padding: 0.75rem 1.5rem; border: 2px solid var(--user-border); background: white; color: var(--user-text); border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s;">
                    <i class="fas fa-laptop"></i> 2 Devices
                </button>
                <button class="device-tab-btn" data-devices="3"
                    style="padding: 0.75rem 1.5rem; border: 2px solid var(--user-border); background: white; color: var(--user-text); border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s;">
                    <i class="fas fa-tv"></i> 3 Devices
                </button>
                <button class="device-tab-btn custom-btn" data-devices="custom"
                    style="padding: 0.75rem 1.5rem; border: 2px dashed #8B0000; background: rgba(139, 0, 0, 0.05); color: #8B0000; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s;">
                    <i class="fas fa-users"></i> Custom (4+)
                </button>
            </div>
            <p style="margin-top: 1rem; color: var(--user-text-light); font-size: 0.9rem;">
                <i class="fas fa-info-circle"></i> Prices update based on number of devices selected. Need 4+ devices?
                <a href="/user/support" style="color: #8B0000; font-weight: 600;">Contact us</a>
            </p>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.5rem;"
            ...
            <?php foreach ($packages as $package):
                ...
                // Get price from pricing table for 1 device (default)
                $displayPrice = PricingCalculator::getPackagePrice($package['duration_days'], 1);
                if (!$displayPrice) {
                    $displayPrice = $package['price'];
                }
                ?>
                <div
                    style="border: 2px solid <?php echo $isCurrentPackage ? 'var(--user-primary)' : 'var(--user-border)'; ?>; border-radius: 12px; padding: 1.5rem; position: relative; transition: all 0.3s ease;">
                    <?php if ($isCurrentPackage): ?>
                        <div
                            style="position: absolute; top: -10px; right: 20px; background: var(--user-primary); color: white; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.75rem; font-weight: 600;">
                            CURRENT
                        </div>
                    <?php endif; ?>
                    ...
                    <div style="margin-bottom: 1.5rem;">
                        <div style="font-size: 2rem; font-weight: bold; color: var(--user-primary);" class="package-price"
                            data-package-id="<?php echo $package['id']; ?>"
                            data-duration-days="<?php echo $package['duration_days']; ?>"
                            data-months="<?php echo $months; ?>">
                            KES <span class="price-amount"><?php echo number_format($displayPrice, 0); ?></span>
                        </div>
                        <div style="color: var(--user-text-light); font-size: 0.9rem;">
                            <?php echo $months; ?> Month<?php echo $months > 1 ? 's' : ''; ?> • <span class="device-count">1
                                Device</span>
                        </div>
                    </div>
                    ...
                    <a href="/user/subscriptions/subscribe?package=<?php echo $package['id']; ?>&devices=1"
                        class="btn btn-primary subscribe-btn" style="width: 100%; text-align: center;"
                        data-package-id="<?php echo $package['id']; ?>">
                    ...
                    <tbody>
                        <?php foreach ($subscriptionHistory as $sub):
                            ...
                            <tr>
                                ...
                                <td>
                                    <span
                                        style="background: <?php echo $statusColor; ?>; color: white; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.85rem; font-weight: 600; text-transform: uppercase;">
                                        <?php echo $status; ?>
                                    </span>
                                </td>
                            </tr>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Fixed pricing table based on devices and duration (matches admin prices)
        const pricingTable = {
            1: { 1: 1000, 6: 15000, 12: 27600 },      // 1 Device
            2: { 1: 2000, 6: 30000, 12: 55200 },      // 2 Devices  
            3: { 1: 3000, 6: 45000, 12: 82800 }       // 3 Devices
        };
        ...
        function updatePrices(devices) {
            ...
            subscribeButtons.forEach(btn => {
                ...
                btn.href = `/user/subscriptions/subscribe?package=${packageId}&devices=${devices}`;
            });
        }
        ...
        deviceButtons.forEach(button => {
            button.addEventListener('click', function () {
                ...
                if (devicesValue === 'custom') {
                    window.location.href = '/user/support?inquiry=custom_package';
                    return;
                }
