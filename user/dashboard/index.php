...
<div class="user-card" style="margin-bottom: 1.5rem;">
    <div class="card-body">
        <div class="alert alert-<?php echo $messageType; ?>" style="display: flex; align-items: center; gap: 0.5rem; padding: 1rem; border-radius: var(--user-radius); background: <?php echo $messageType === 'success' ? '#D1FAE5' : '#FEE2E2'; ?>; color: <?php echo $messageType === 'success' ? '#065F46' : '#991B1B'; ?>;">
    ...
    <div class="card-body">
        <?php if ($subscription): ?>
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 2rem;">
                <div>
                    <h3 style="color: var(--user-text); font-size: 1.5rem; margin-bottom: 0.5rem;"><?php echo htmlspecialchars($subscription['package_name']); ?></h3>
                    <p style="font-size: 2rem; font-weight: 700; color: var(--user-primary); margin-bottom: 0.5rem;"><?php echo $subscription['currency']; ?> <?php echo number_format($subscription['price'], 2); ?></p>
                    <p style="color: var(--user-text-light); margin-bottom: 0;"><?php echo $subscription['duration_days']; ?> days</p>
                </div>
                <div style="text-align: right;">
                    <span style="padding: 0.5rem 1rem; border-radius: 20px; font-weight: 600; text-transform: uppercase; font-size: 0.875rem; background: <?php echo $subscription['status'] === 'active' ? '#D1FAE5' : '#FEE2E2'; ?>; color: <?php echo $subscription['status'] === 'active' ? '#065F46' : '#991B1B'; ?>;">
                ...
                <table class="user-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span style="padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; background: <?php echo $payment['status'] === 'completed' ? '#D1FAE5' : ($payment['status'] === 'pending' ? '#FEF3C7' : '#FEE2E2'); ?>; color: <?php echo $payment['status'] === 'completed' ? '#065F46' : ($payment['status'] === 'pending' ? '#92400E' : '#991B1B'); ?>;">
            ...
            <?php if (!empty($notifications)): ?>
                <div style="space-y: 1rem;">
                    ...
                    <div style="display: flex; gap: 1rem; padding: 1rem 0; border-bottom: 1px solid var(--user-border);">
                        <div style="width: 40px; height: 40px; border-radius: 50%; background: rgba(139, 0, 0, 0.1); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                            <i class="fas fa-<?php echo $notification['type'] === 'success' ? 'check-circle' : 'info-circle'; ?>" style="color: var(--user-primary);"></i>
                        </div>
                        <div style="flex: 1;">
                            <h4 style="margin: 0 0 0.25rem 0; color: var(--user-text);"><?php echo htmlspecialchars($notification['title']); ?></h4>
                            <p style="margin: 0 0 0.5rem 0; color: var(--user-text-light); font-size: 0.875rem;"><?php echo htmlspecialchars($notification['message']); ?></p>
                            <span style="font-size: 0.75rem; color: #A0AEC0;"><?php echo timeAgo($notification['created_at']); ?></span>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
...
<div class="user-card">
    ...
    <div class="card-body">
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 2rem;">
            ...
            <div style="background: white; border-radius: var(--user-radius); padding: 2rem; box-shadow: var(--user-shadow); border: 1px solid var(--user-border);">
                <div style="text-align: center; margin-bottom: 1.5rem;">
                    <h3 style="color: var(--user-text); font-size: 1.5rem; margin-bottom: 1rem;"><?php echo htmlspecialchars($package['name']); ?></h3>
                    <div style="display: flex; align-items: baseline; justify-content: center; gap: 0.25rem;">
                        <span style="font-size: 1.25rem; color: var(--user-text-light);"><?php echo $package['currency']; ?></span>
                        <span style="font-size: 2.5rem; font-weight: 700; color: var(--user-primary);"><?php echo number_format($package['price'], 2); ?></span>
                    </div>
                </div>
                <div style="margin-bottom: 2rem;">
                    <p style="color: var(--user-text-light); margin-bottom: 1.5rem; text-align: center;"><?php echo htmlspecialchars($package['description']); ?></p>
