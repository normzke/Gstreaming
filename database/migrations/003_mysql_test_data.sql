...
((SELECT id FROM users WHERE username = 'testuser1'), 1, 'active', NOW(), DATE_ADD(NOW(), INTERVAL 30 DAY), true),
-- Expired subscription for testuser2
((SELECT id FROM users WHERE username = 'testuser2'), 2, 'expired', DATE_SUB(NOW(), INTERVAL 60 DAY), DATE_SUB(NOW(), INTERVAL 30 DAY), false),
...
((SELECT id FROM users WHERE username = 'testuser1'), 
 ...
 500.00, 'mpesa', '254712345678', 'completed', DATE_SUB(NOW(), INTERVAL 5 DAY)),
...
((SELECT id FROM users WHERE username = 'testuser3'), 
 ...
 1500.00, 'mpesa', '254712345680', 'failed', DATE_SUB(NOW(), INTERVAL 2 DAY)),

-- Pending payment for testuser3
((SELECT id FROM users WHERE username = 'testuser3'), 
 (SELECT id FROM user_subscriptions WHERE user_id = (SELECT id FROM users WHERE username = 'testuser3') LIMIT 1),
 1500.00, 'mpesa', '254712345680', 'pending', DATE_SUB(NOW(), INTERVAL 1 HOUR))
ON DUPLICATE KEY UPDATE user_id = user_id;
...
((SELECT id FROM users WHERE username = 'testuser1'), 
 (SELECT id FROM user_subscriptions WHERE user_id = (SELECT id FROM users WHERE username = 'testuser1') LIMIT 1),
 CONCAT('http://localhost:4000/GStreaming/stream/testuser1_', UNIX_TIMESTAMP()),
 'testuser1_stream', 'stream_pass_123', true)
ON DUPLICATE KEY UPDATE user_id = user_id;

-- Update user last login times for testing
UPDATE users SET last_login = DATE_SUB(NOW(), INTERVAL 1 DAY) WHERE username = 'testuser1';
UPDATE users SET last_login = DATE_SUB(NOW(), INTERVAL 5 DAY) WHERE username = 'testuser2';
UPDATE users SET last_login = DATE_SUB(NOW(), INTERVAL 30 DAY) WHERE username = 'testuser3';
